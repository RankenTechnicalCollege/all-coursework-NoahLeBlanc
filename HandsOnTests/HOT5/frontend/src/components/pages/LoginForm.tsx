...
function LoginForm() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      throw new Error(res.data?.error?.message || "Google sign-in failed");
    } catch (err: any) {
      ...
      setError(errorMessage);
      setGoogleLoading(false); // Only reset if error, not on redirect
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="w-full max-w-md p-6 bg-white rounded-lg shadow-md">
        <form onSubmit={handleSubmit}>
          <FieldSet>
            ...
            <div className="flex items-center gap-3 mb-6">
              <img className="w-10 h-10" src="/login-3-svgrepo-com.svg" alt="Login" />
        ...
        <div className="bg-gray-100 mt-6 p-3 rounded-lg flex justify-center items-center">
          <p className="text-gray-700 mr-2">Not a user?</p>
          <Link 
